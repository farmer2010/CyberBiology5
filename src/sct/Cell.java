...
public class Cell{
	...
	public int killed = 0;
	public int[][] map;
	public int[][] commands = new int[16][9];
	private int index = 0;
	public int state;//0 - отросток, 1 - лист/древесина, 2 - семечко
	...
	public void update_commands(ListIterator<Cell> iterator) {
		int[] c = {commands[index][4], commands[index][5]};
		if (c[0] > 17) {//нет условия
			multiply(iterator);
		}else if (condition(c)){//условие выполнено
			cmd(commands[index][6]);
		}else {//условие не выполнено
			cmd(commands[index][7]);
		}
	}
	public void cmd(int command) {
		if (command == 0) {
			//ничего не делать
		}else if (command == 1) {//превратиться в семечко
			state = 2;
			energy = 300;
			my_tree.energy -= 300;
			my_tree = null;
	...
	public void multiply(ListIterator<Cell> iterator) {
		...
		my_tree.cells_count++;
		for (int i = 0; i < 4; i++) {
			if (commands[index][i] < 32) {
				int[] pos = get_rotate_position(i);
				if (pos[1] > 0 & pos[1] < world_scale[1]) {
					if (map[pos[0]][pos[1]] == 0) {
						int[][] new_commands = new int [16][9];
						for (int g = 0; g < 16; g++) {
							for (int j = 0; j < 9; j++) {
						...
						new_cell.index = commands[index][i] % 16;
	...
	public boolean condition(int[] c) {
		return(false);
	}
}
